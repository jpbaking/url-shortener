...
- Check logs: `docker compose logs -f <service>` where `<service>` is `nginx`, `backend`, or `postgres`.
...
- [playwright/](playwright/AGENTS.md) — Playwright E2E suite; 41 tests covering the API, redirect handler, and SPA; runs against the live Docker Compose stack
